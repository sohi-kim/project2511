...
# LangChain 1.x 
#     ㄴ pip install langchain_huggingface pdfplumber sentence-transformers
...
# 챗봇 프롬프트를 템플릿 형태로 구성
# 입력을 그대로 출력하는 단순한 Runnable 객체입니다. 파이프라인(체인) 구성 시 유용
...
load_dotenv()
# from langchain_core.embeddings import Embedding 상속 받아서 Embedding 객체로 정의한 클래스
#  ↙ KoreanEmbeddings 의 속성 : embeddings
...
# (1)pdf 를 텍스트로 변환 -> 임베딩 -> 벡터 db에 저장
# (2)벡터 db에 저장된 인덱스에 조회 -> llm 에 전달하여 사용자 응답
#   ↙ PdfRAGSystem 의 속성 : index_name 문자열, pc(파인콘),embeddings,llm,vectorstore 객체
class PdfRAGSystem:
    ...
    def create_index(self, dimension: int = 1024):
        ...
        # 기존 인덱스에 있는 index_name 인지 검사. 없을 때만 인덱스 생성
        ...
        else:  # index_name 이 존재한다면
            print(f"Index '{self.index_name}' already exists.")

    # 청킹을 위해 splitter를 생성 -> PDF 텍스틑 청킹 -> 청크 리턴
    def load_and_split_pdf(
        ...
        Args:
            ...
            chunk_overlap: 청크 오버랩(chunk_size 의 10~20%)
        """
        print(f"Loading PDF: {pdf_path}")
        
        # PDF 로더 
        loader = PDFPlumberLoader(pdf_path)
        # loader 를 이용하여 텍스트 추출 -> Document 타입 객체로 변환
    ...
    def create_rag_chain(self):
        ...
        template = """당신은 제품 요리 레시피 책 기반 질의응답 전문가입니다.
...
- 필요하면 단계별 설명을 제공하세요.예를 들면,요리 방법과 재료를 구분하여 설명하세요.
...
def main(run:bool,pdf_path:str,index_name:str,questions:List[str]):
    ...
    # 새로운 문서 임베딩하기
    if run:  # 참이면 index_name 에 대해 업서트(create_vectorstore 메소드 실행)
